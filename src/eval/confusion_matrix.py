...
confusion_matrix.py
-------------------
Runs the FULL collision detector pipeline (spark confirmation) on clips
in classified/<class>/ folders and evaluates PEDAL classification only.

Each clip's predicted label = the first SPARK_CONFIRMED pedal, or "miss".
Hole events are intentionally ignored — holes run in the background to
keep ball-tracking state accurate, but never influence the predicted label.

Ground truth = subfolder name (Pedal_1 … Pedal_N, miss).

Usage:
    python3 src/eval/confusion_matrix.py --config config.json --classified-dir ./test/results/classifiedFlash
"""
...
def global_spark_intensity(frame):
    """
    Measure yellow/white spark pixel ratio in the bottom 50% of the frame.
    Used to detect when a spark explosion is happening so we can suppress
    ball detection (spark particles confuse find_ball_candidate).
    """
    h = frame.shape[0]
    region = frame[h // 2:, :]
    hsv = cv2.cvtColor(region, cv2.COLOR_BGR2HSV)
...
# Intensity threshold above which we consider the frame "spark-saturated"
# and skip updating the Kalman with a new detection.
# From analysis: clean frames ~0.005, spark frames 0.05–0.22.
SPARK_SUPPRESS_THRESHOLD = 0.04
...
def classify_clip_full(video_path, pedals, holes,
    ...
    Fix 1 — Spark suppression:
        When global yellow intensity in the bottom half of the frame exceeds
        SPARK_SUPPRESS_THRESHOLD, spark particles flood find_ball_candidate
        and make the tracker jump to wrong positions. During these frames we
        skip the Kalman correction and use pure prediction instead, keeping
        the ball position stable at its pre-impact trajectory.

    Fix 2 — spark_confirm_frames default lowered to 1:
        Some throws produce a genuine but very brief spark (single frame).
        Requiring 3 consecutive frames was too strict and caused misses.
        Default is now 1; pass --spark-frames 3 to restore old behaviour.
    """
    ...
    # ── Pedal state (mirrors collision_detector.py exactly) ──
    pedal_state = [{
        "spark_streak":     0,
    ...
    while True:
        ...
        candidate = find_ball_candidate(fg_mask) if not spark_active else None
        ball_pos  = None
        ...
        if candidate:
            ...
            if kalman_initialized and (missed_frames < MAX_MISSED or spark_active):
                ...
                kalman_initialized = False
                # Reset all state on ball loss (same as collision_detector.py)
